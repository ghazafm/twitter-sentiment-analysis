...
def clean_text(data, tag=False):
    ...
    print("clean_text")
    data["clean_text"] = data["Tweet"].apply(clear_non_word)
    data['clean_text'].to_csv('oke.csv')
    print("no_double")
    ...
    if tag:
        ...
        tokenize_data = tokenize_data.apply(clean_manual, token=True,tag=True)
